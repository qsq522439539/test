...
class MessageBoxDlg(QDialog):
	def __init__(self,parent=None):
		...
		questionButton=QPushButton("Question")             #Question：为正常的操作提供一个简单的询问。   Information：为正常的操作提供一个提示。
		informationButton=QPushButton("Information")       #Warning：提醒用户发生了一个错误。          Critical：警告用户发生了一个严重错误。
	...
	def slotInformation(self):
		QMessageBox.information(self,"Information",self.tr("填写任意想告诉于用户的信息!")),self.label.setText("Information MessageBox")
